...
@router.post(
    ...
    status_code=status.HTTP_303_SEE_OTHER,
    tags=["Web"],
    summary="Atualizar jogo via Formulário Web"
)
def editar_web(
    id: int,
    titulo: Annotated[str, Form(description="Título do jogo")],
...
@router.post(
    ...
    status_code=status.HTTP_303_SEE_OTHER,
    tags=["Web"],
    summary="Alternar status do jogo via Formulário Web"
)
def alternar_status_web(
    id: int,
    db: Session = Depends(get_db)
):
    jogo = db.query(Jogo).filter(Jogo.id == id).first()
...
@router.patch(
    "/api/jogos/{id}/concluir",
    response_model=JogoResponse,
    tags=["API REST - Jogos"],
    summary="Alternar status zerado/jogando"
)
def api_alternar_status(
    id: int,
    db: Session = Depends(get_db)
):
    jogo = db.query(Jogo).filter(Jogo.id == id).first()
